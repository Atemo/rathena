	...
	if (this->nodeExists(node, "Tree")) {
		for (const auto &it : node["Tree"]) {
			...
			if (this->nodeExists(it, "BaseLevel")) {
				...
				if (!this->asUInt16(it, "BaseLevel", baselv))
					return 0;
			...
			if (this->nodeExists(it, "JobLevel")) {
				...
				if (!this->asUInt16(it, "JobLevel", joblv))
					return 0;
...
void SkillTreeDatabase::loadingFinished() {
	for (auto &data : *this) {
		...
		inherit_job = data.second->inherit_job;

		uint16 baselv_max = static_cast<uint16>(pc_class_maxbaselv(data.first));
		uint16 joblv_max = static_cast<uint16>(pc_class_maxjoblv(data.first));

		for (uint16 i = 0; i < 10 && i < inherit_job.size(); i++) {
			std::shared_ptr<s_skill_tree> tree = this->find(inherit_job[i]);
			if (tree == nullptr || tree->skills.empty())
				break;
			...
			for (const auto &it : tree->skills) {
				...
				data.second->skills.insert({ it.first, it.second });
				std::shared_ptr<s_skill_tree_entry> skill = data.second->skills[it.first];
			...
			if (!tree->inherit_job.empty()) {
				for (const auto &it : tree->inherit_job) {
					if (!util::vector_exists(inherit_job, it))
						inherit_job.push_back(it);	// check the childs
				}
			}
		}
		data.second->inherit_job.clear();	// skill tree completed: no need to re-add the skills from inherit if the job is set for another tree
	}
