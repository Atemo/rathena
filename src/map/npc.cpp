...
int npc_ontouch_event(struct map_session_data *sd, struct npc_data *nd)
{
	...
	// The area condition is according to the OnTouch* type label, if exists.
	// if( pc_ishiding(sd) )
...
int npc_touch_areanpc(struct map_session_data* sd, int16 m, int16 x, int16 y)
{
	...
	for (i = 0; i < map[m].npc_num_area; i++) {
		...
		if (x >= map[m].npc[i]->bl.x - xs && x <= map[m].npc[i]->bl.x + xs && y >= map[m].npc[i]->bl.y - ys && y <= map[m].npc[i]->bl.y + ys) {
			...
			case NPCTYPE_SCRIPT:
				...
				if (npc_ontouch_event(sd, map[m].npc[i]) > 0 && npc_ontouch2_event(sd, map[m].npc[i]) > 0) { // failed to run OnTouch event, so just click the npc
					int k;

					ARR_FIND(0, sd->areanpc.count, k, sd->areanpc.ids[k] == map[m].npc[i]->bl.id);
					...
					if (k == sd->areanpc.count) {
						...
						sd->areanpc.ids[sd->areanpc.count] = map[m].npc[i]->bl.id;
						sd->areanpc.count++;
					}
